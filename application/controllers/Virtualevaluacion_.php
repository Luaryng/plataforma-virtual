...
class Virtualevaluacion extends Sendmail{
            ...
            if (($_SESSION['userActivo']->tipo == 'AD') || ($_SESSION['userActivo']->tipo == 'DA') || ($_SESSION['userActivo']->tipo == 'DC')) {
                ...
                else
                {
                    ...
                    if ($pgcodpg=="0"){
                        ...
                        if ((($pgimgactual!="") && ($subio_imagen==TRUE))|| (($pgimgactual!="") && ($sin_imagen=="SI"))){
                            $pathtodir =  getcwd() ; 
                    ...
                    elseif ($tvencio=="SI") {
                        ...
                        if ($excodentrega=="0"){
                            ...
                            if ($filarp->salida=='1'){
                                ...
                                // AUDITORIA
                                $rpse= $this->mvirtualevaluacion->m_get_respuestas_entregadas_x_evaluacion_view(array(base64url_decode($codmaterial),$exmiembro));
                                $textrpta = "";
                                $valrpta = array();
                                foreach ($rpse as $key => $prpt) {
                                    $valrpta[] = "pregunta: ".$prpt->pregunta." - respuesta: ".$prpt->enunciado;
                                }
                                $respts = implode("','", $valrpta);
                                $textrpta = $respts;
                                $contenido = $_SESSION['userActivo']->usuario." - ".$_SESSION['userActivo']->paterno." ".$_SESSION['userActivo']->materno." ".$_SESSION['userActivo']->nombres.", está guardando su examen en la tabla TB_VIRTUAL_EVALUACION_ALUMNO COD.".$filarp->nid.", marcando como respuestas: $textrpta";
                                $auditoria = $this->mauditoria->insert_datos_auditoria(array($usuario, $fictxtaccion, $contenido, $sede));
                                // FIN AUDITORIA
    ...
    public function vw_evaluaciones_individual_respuestas()
    {
        $dataex['status'] =FALSE;
        $dataex['msg']    = '¿Que Intentas?.';
        ...
        $division=base64url_decode($this->input->post('ex-division'));
        $codcarga=base64url_decode($this->input->post('ex-carga'));
        $codmaterial=base64url_decode($this->input->post('ex-cod'));
        $codmiembro = base64url_decode($this->input->post('ex-miembro'));
        $arraymc['curso'] = $this->mcargasubseccion->m_get_carga_subseccion(array($codcarga,$division));
        if (isset($arraymc['curso']->codcarga)){
            $mat = $this->mvirtual->m_get_material(array($codmaterial));
            if (isset($mat->codigo)){
                ...
                $pgts= $this->mvirtualevaluacion->m_get_preguntas_x_evaluacion(array($codmaterial));
                $rps= $this->mvirtualevaluacion->m_get_respuestas_x_evaluacion(array($codmaterial));
                $rpse= $this->mvirtualevaluacion->m_get_respuestas_entregadas_x_evaluacion_view(array($codmaterial,$codmiembro));
                // $respuestas=array();
                $ntotal=0;
                foreach ($pgts as $kpg => $pgt) {
                    // $pgt->rpts=array();
                    ...
                    elseif ($pgt->codtipo==4) {
                        ...
                        foreach ($rpse as $key => $rpe) {
                            if (($pgt->codpregunta==$rpe->codpregunta)){
                                $pgt->enunciadorpt = $rpe->enunciado;
                                unset($rpse[$key]);
                            }
                            // $pgt->rpts[]=$rpts;
                        }
                ...
                $this->load->model('mmiembros');
                $datamiembro = $this->mmiembros->m_miembrosxcodigo(array($codmiembro));
                $arraymc['datomiembro'] = $datamiembro;

                $preview = "<div class='container-fluid'>
                            <div class='col-12 mb-2 text-left'>
                                <span class='h6 font-weight-bold border-bottom border-dark'>Sus respuestas registradas son:</span>
                            </div>";
                $nropregunta=0;
                $ptotal=0;
                foreach ($pgts as $key => $pregunta) {
                    $nropregunta++;
                    $rp = array();
                    ...
                    $codpregunta=base64url_encode($pregunta->codpregunta);
                    $pgvalor=floatval($pregunta->valor);
                    
                    $venunciado=htmlspecialchars(stripslashes($pregunta->enunciado));
                    $venunciadox=htmlspecialchars(stripslashes($pregunta->enunciadox));
                    $preview .= "<div class='mb-2 text-left'>
                            <div class='col-12'>
                                <span class='font-weight-bold'>$nropregunta .- $venunciado</span>
                            </div>
                            <div class='col-12'>
                                <small class='font-weight-bold'>$venunciadox</small>
                            </div>

                            <div class='divrptas' data-contador='0'>";
                                    $nro=0;
                                    $tipocheck="radio";
                                    if ($pregunta->codtipo==4){
                                        $tipocheck="checkbox";
                                    }
                                    if ($pregunta->codtipo==7){
                                        $rptexto=htmlspecialchars(stripslashes($pregunta->texto));
                                        $preview .= "<div class='col-12'>
                                                <span class=''>$rpenunciadop</span>
                                            </div>";
                                    ...
                                    else
                                    {
                                        ...
                                        $rpenunciadop=htmlspecialchars(stripslashes($pregunta->enunciadorpt));
                                        $preview .= "<div class='col-12 input-group input-group-sm mb-1'>
                                                        <div class='input-group-prepend'>
                                                            <div class=''>
                                                              <span><i class='far fa-dot-circle text-primary'></i></span>
                                                            </div>
                                                        </div>
                                                        <span class='ml-3'>$rpenunciadop</span>
                                                    </div>";
        ...
        $dataex['vieweval'] = $preview;
        $dataex['preguntas'] = $pgts;
