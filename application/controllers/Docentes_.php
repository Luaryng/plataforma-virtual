...
class Docentes extends CI_Controller {
	...
	public function vw_docentes(){
		...
		$arr_vwdocentes=$this->mdocentes->m_filtrar_trabajadores(array("%"));
		$arr_vwdocentes['departamentos'] =$this->mubigeo->m_departamentos();
		$this->load->model('marea');
		$arr_vwdocentes['areas'] =$this->marea->m_get_areas($_SESSION['userActivo']->idsede);
	...
	public function fn_filtrar_trabajadores()
	{
		...
		if ($this->input->is_ajax_request())
		{
			...
			else
			{
				...
				$cuentas=$this->mdocentes->m_filtrar_trabajadores(array("%".$busqueda.'%'));
	...
	public function fn_guardar_datos_docente()
	{
		...
		if ($this->input->is_ajax_request())
		{
			...
			else
			{
				...
				// $strabaja = $this->input->post('fidptrabaja');
				// $lugartrabaja = $this->input->post('fidplugtrab');
				$lugarnacim = $this->input->post('fidplugnacim');
				// $nompadre = $this->input->post('fidpnompadre');
				// $nommadre = $this->input->post('fidpnommadre');
				$celular2 = $this->input->post('fidpcelular2');
				// $secundaria = $this->input->post('fidpsecundaria');
				$estadocivil = $this->input->post('fidpstadociv');

				$this->load->model('mpersona');	
				if ($fidpid==""){
					$dataex['msg'] ="Ocurrio un error al momento de registrar una nueva persona";
					$newcod=$this->mpersona->insert_datos_personales_docente(array($fitxtcodinstitucional,$ficbtipodoc,$fitxtdni,$fitxtapelpaterno,$fitxtapelmaterno,$fitxtnombres,$ficbsexo,$fitxtfechanac, $lugarnacim,$fitxtcelular,$fitxttelefono,$fitxtemailpersonal,$fitxtdomicilio,$ficbdistrito,$fitxtdomiciliootro,'4', $estadocivil, $celular2));
				}else{
					$dataex['msg'] ="Ocurrio un error al momento de actualizar a una persona";
					$newcod=$this->mpersona->update_datos_personales_docente(array(base64url_decode($fidpid),$fitxtcodinstitucional,$ficbtipodoc,$fitxtdni,$fitxtapelpaterno,$fitxtapelmaterno,$fitxtnombres,$ficbsexo,$fitxtfechanac, $lugarnacim,$fitxtcelular,$fitxttelefono,$fitxtemailpersonal,$fitxtdomicilio,$ficbdistrito,$fitxtdomiciliootro, '4', $estadocivil, $celular2));
				}
				...
				if ($newcod>0){
					...
					if ($fitxtcoddocente==""){
						...
						$coddocente=str_pad(base64url_decode($fidpid), 5, "0", STR_PAD_LEFT);
						$idsede=$_SESSION['userActivo']->idsede;
						$newcod=$this->mdocentes->m_insert(array($coddocente,base64url_decode($fidpid),$fitxteinstitucional,$ficbtipo,$fitxtcargo,$idsede,$ficbidarea));
						if ($newcod==1){
							$dataex['status'] =TRUE;
