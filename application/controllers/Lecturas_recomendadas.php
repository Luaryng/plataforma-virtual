...
class Lecturas_recomendadas extends Error_views{

	function __construct(){
		parent::__construct();
		$this->load->helper("url"); 
		$this->load->model("mrepositorio");
		$this->load->model("mlecturas");
		$this->load->model('mcategoria_transparencia');
		//$this->load->library('pagination');
	}


    public function vw_lecturas_archivos($area,$id)
    {
        $area=base64url_decode($area);
        $id=base64url_decode($id);
        $fila=$this->mlecturas->m_get_item(array($id,$area));
	...
	{
		if (getPermitido("86")=='SI'){
			$tipo=$this->input->get('lr');
			$ahead= array('page_title' =>'Lecturas Recomendadas | ERP'  );
			$asidebar= array('menu_padre' =>'lecturas');
			$this->load->view('head',$ahead);
			$this->load->view('nav');
			$this->load->view('sidebar_portal',$asidebar);
			$items=array();
			if ($tipo!="")  $items= $this->mrepositorio->m_get_items(array($tipo,'LR','%'));
			$items = $this->mrepositorio->m_get_items(array($tipo,'LR','%'));
			$categorias = $this->mcategoria_transparencia->m_filtrar_categorias_activasxtipo(array('LR'));
			$this->load->view('lecturas/index', array('items' => $items, 'categorias' => $categorias ));
	...
	public function fn_search_data()
	{
		...
		if($this->input->is_ajax_request()){
			...
			$rstdata = $this->mlecturas->m_get_items(array($busqueda,'LR','%'.$titulo.'%'));
			if (count($rstdata) > 0) {
                $dataex['status'] = true;
                $rsdata['items'] = $rstdata;
                $datos = $this->load->view('lecturas/result_data', $rsdata, true);
            } else {
	...
	{
		if (getPermitido("87")=='SI'){
			$ahead= array('page_title' =>'Lecturas Recomendadas - Agregar | ERP'  );
			$asidebar= array('menu_padre' =>'lecturas');
			$this->load->view('head',$ahead);
			$this->load->view('nav');
			$this->load->view('sidebar_portal',$asidebar);
			$arraydts['categorias'] =$this->mcategoria_transparencia->m_filtrar_categorias_activasxtipo(array('LR'));
			$this->load->view('lecturas/vw_agregar', $arraydts);
			$this->load->view('footer');
		}
		else{
			 $this->vwh_nopermitido("NO AUTORIZADO - ERP");
	...
	public function vw_editar($area,$id)
    {
		...
		if (getPermitido("88")=='SI'){
			$area=base64url_decode($area);
        	$id=base64url_decode($id);
			$ahead= array('page_title' =>'Lecturas Recomendadas - Editar | ERP'  );
			$asidebar= array('menu_padre' =>'lecturas');
			$this->load->view('head',$ahead);
			$this->load->view('nav');
			$this->load->view('sidebar_portal',$asidebar);
			$fila['mat']=$this->mrepositorio->m_get_item(array($id,$area));
			$fila['categorias'] =$this->mcategoria_transparencia->m_filtrar_categorias_activasxtipo(array('LR'));
			$this->load->view('lecturas/vw_agregar', $fila);
			$this->load->view('footer');
		}
		else{
			 $this->vwh_nopermitido("NO AUTORIZADO - ERP");
		...
		{	
			if (getPermitido("87")=='SI'){
				...
				else
				{
	        		...
	        		if ($codigo=="0"){
	        			$rpta=$this->mlecturas->m_insert_lecturas(array($titulo, $descripcion, $link,$peso,$tipofile, $categoria, $torden,$area));
	        		}
	        		else{
	        			$rpta=$this->mlecturas->m_update_lecturas(array(base64url_decode($codigo),$titulo, $descripcion, $link,$peso,$tipofile, $categoria, $torden,$area));
	        		}
	        		
	        		if ($rpta->salida==1){
	        			$dataex['status'] =TRUE;
						$dataex['msg'] ="Datos registrados correctamente";
						$dataex['redirect'] =base_url()."portal-web/lecturas-recomendadas?lr=$categoria";
	        		}
	...
	public function fn_delete_file(){
        ...
        if ($this->input->is_ajax_request()) {
            if ($_SESSION['userActivo']->tipo != 'AL'){
                ...
                $this->mlecturas->m_update_delfile_lecturas(array($codigo));
                $dataex['status'] =true;
        ...
        if ($this->input->is_ajax_request()) {
        	...
        	if (getPermitido("89")=='SI'){
	            if ($_SESSION['userActivo']->tipo != 'AL'){
	                ...
	                $fila=$this->mlecturas->m_get_item(array($codigo,$area));
	                if (isset($fila->codigo)){
	                	$this->mlecturas->m_delete_lecturas(array($codigo));
	                	$dataex['status'] =true;
		                if ("" !== $fila->ruta){
		                    $link    = $fila->ruta;
		                    $pathtodir =  getcwd() ; 
