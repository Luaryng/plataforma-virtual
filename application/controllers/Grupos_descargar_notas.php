...
class Grupos_descargar_notas extends CI_Controller {
	...
	public function vw_notas_principal()
    {
        if (getPermitido("128")=='SI'){
    		...
    		$this->load->view('grupos/descargarnotas/vw_notas_descarga',$a_ins);
    ...
    public function fn_get_carga_por_grupo_notas()
    {
        ...
        if ($this->input->is_ajax_request())
        {
            ...
            else
            {
                ...
                $cursos = $this->load->view('grupos/descargarnotas/vw_grupo_result',$vcursos, true);
                $dataex['status'] =TRUE;
                $dataex['vdata'] =$cursos;
            }
    ...
    public function fn_get_alumnos_notas()
    {
        ...
        if ($this->input->is_ajax_request())
        {
            ...
            else
            {
                ...
                $valumnos = $this->mnotas_descarga->m_notas_carga_subseccion(array($codcarga,$coddivision));
                $in = 0;
                
                for ($i=0; $i < count($valumnos) ; $i++) { 

                    
                    if ($valumnos[$i]->codmatnota==""){
                        $in = $in - 1;
                         $valumnos[$i]->codmatnota_64 = "$in";
                    }
                    else{
                        $valumnos[$i]->codmatnota_64 = base64url_encode($valumnos[$i]->codmatnota);;
                    }
                    $valumnos[$i]->idmat_64 = base64url_encode($valumnos[$i]->idmat);
                    $valumnos[$i]->idcarga_64 = base64url_encode($valumnos[$i]->idcarga);
                    $valumnos[$i]->iddivision_64 = base64url_encode($valumnos[$i]->iddivision);
                    //$valumnos[$i]->coddocente_64 = base64url_encode($valumnos[$i]->coddocente);
                    $valumnos[$i]->idcarrera_64 = base64url_encode($valumnos[$i]->idcarrera);
                    $valumnos[$i]->idmiembro_64 = base64url_encode($valumnos[$i]->idmiembro);
    ...
    public function fn_get_alumnos_notas_finales()
    {
        $this->form_validation->set_message('required', '%s Requerido');
        $this->form_validation->set_message('min_length', '* {field} debe tener al menos {param} caracteres.');
        $this->form_validation->set_message('is_unique', '* {field} ya se encuentra registrado.');
        ...
        if ($this->input->is_ajax_request())
        {
            ...
            if (getPermitido("128")=='SI')
            {
                ...
                else
                {
                    ...
                    $miembro = base64url_decode($this->input->post('fmt-codmiembro'));
                    ...
                    $fechamatricula = $this->input->post('fmt-fechamatricula');
                    
                    $dataex['matnota'] = '0';
                    $dataex['msg'] ="Antes de guardar";
                    if (intval($this->input->post('fmt-codmatnota')) < 0){
                        $dataex['msg'] ="Antes de insertar";
                        ...
                        $dataex['matnota']=base64url_encode($rpta->newcod);
                        $contenido = $_SESSION['userActivo']->usuario." - ".$_SESSION['userActivo']->paterno." ".$_SESSION['userActivo']->materno." ".$_SESSION['userActivo']->nombres.", está agregando una matricula en la tabla TB_MATRICULA_CURSOS_NOTA_FINAL COD.".$rpta->newcod;
                         if ($rpta->salida == 1){
                            ...
                            $dataex['status'] =TRUE;
                            $dataex['msg'] ="Datos insertados correctamente";
                            //$dataex['idmatricula'] = base64url_encode($codmatricula);
                            
                        }
                    }
                    else {
                        $dataex['msg'] ="Antes de editar";
                        $dataex['matnota']=$this->input->post('fmt-codmatnota');
                        $codmatnota = base64url_decode($this->input->post('fmt-codmatnota'));
                        $rpta=$this->mnotas_descarga->m_update_notas_descarga(array($codmatnota, $codmatricula, $tipo, $periodo, $carrera, $plan, $ciclo, $turno, $seccion, $fechamatricula, $codcarga, $codsubseccion, $docente, $unidad, $resolucion, $convalfecha, $notafinal, $notarecupera, $observacion, $usuario, $sede, $estado, $repitencia, $fecha));
                        ...
                        if ($rpta->salida == 1){
                            ...
                            $dataex['status'] =TRUE;
                            $dataex['msg'] ="Datos editados correctamente";
                            //$dataex['idmatricula'] = base64url_encode($codmatricula);
            ...
            if (getPermitido("128")=='SI')
            {
                ...
                else
                {
                    ...
                    foreach ($data as $value) {
                        ...
                        $codcarga = base64url_decode($value[3]);
                        $codsubseccion = base64url_decode($value[4]);
                        $periodo = base64url_decode($value[8]);
                        $carrera = base64url_decode($value[6]);
                        $plan = base64url_decode($value[9]);
                        $ciclo = base64url_decode($value[7]);
                        $turno = base64url_decode($value[11]);
                        $seccion = base64url_decode($value[10]);
                        $unidad = base64url_decode($value[12]);
                        $docente = base64url_decode($value[5]);
                        $notafinal = $value[15];
                        $notarecupera = $value[16];
                        $estado = $value[13];
                        $repitencia = $value[14];

                        $fechamatricula = $value[17];
                        ...
                        if (intval($value[2]) < 0) {
                            ...
                            $codmatnota = base64url_decode($value[2]);
                            $rpta=$this->mnotas_descarga->m_update_notas_descarga(array($codmatnota, $codmatricula, $tipo, $periodo, $carrera, $plan, $ciclo, $turno, $seccion, $fechamatricula, $codcarga, $codsubseccion, $docente, $unidad, $resolucion, $convalfecha, $notafinal, $notarecupera, $observacion, $usuario, $sede, $estado, $repitencia, $fecha));
                        ...
                        $datos['idnew']=base64url_encode($rpta->newcod);
                        $datos['notafin'] = $value[15];
                        $datos['notarec'] = $value[16];
                        $datos['estado']=false;
                        if ($rpta->salida == 1){
                            $datos['estado']=true;
            ...
            if (getPermitido("128")=='SI')
            {
                ...
                else
                {
                    ...
                    foreach ($data as $value) {
                        ...
                        $datos['idorg']=$value[2];
                        //APR,DES,DPI,NSP,MTR,REP
                        if (($estado=="REP") || ($estado=="NSP")||($estado=="DPI")){
                            $estado="-";
                        }
                        else{
                            if (!is_null($notarecupera)){
                                if ($notarecupera<12.5){
                                    $estado="DES";
                                }
                                else{
                                    $notarecupera="APR";   
