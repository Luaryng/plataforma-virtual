		...
		if ($this->input->is_ajax_request())
		{
			...
			else
			{
				...
				if ($nrs==1){
					...
					$inscrito = $this->minscrito->m_get_inscrito_por_carne(array($pagantes[0]->codpagante));
					if (@count($inscrito) > 0) {
					...
					$dtpag['historial'] = $pagantes;
					$dataex['vpagantes']=$rspagant = $this->load->view('facturacion/dts_pagante',$dtpag,TRUE);
...
public function get_filtrar_lista()
	{
		...
		if ($this->input->is_ajax_request())
		{
			...
			$busqueda = $this->input->post('fictxtsearch');
			
			$dtpag['historial'] = $this->mpagante->m_get_pagante_search(array("%".$busqueda."%") );
    ...
    public function fneliminar_cliente()
    {
        $dataex['status'] = false;
        $dataex['msg']    = '¿Que intentas? .|.';
        if ($this->input->is_ajax_request()) {
            $this->form_validation->set_message('required', '%s Requerido');
            $dataex['msg'] = 'Intente nuevamente o comuniquese con un administrador.';
            $this->form_validation->set_rules('c-codigo', 'codigo cliente', 'trim|required');
            if ($this->form_validation->run() == false) {
                $dataex['msg'] = validation_errors();
            } else {
                $dataex['msg'] = "Ocurrio un error, no se puede eliminar este cliente";
                $txtcodigo    = base64url_decode($this->input->post('c-codigo'));
                
                $usuario = $_SESSION['userActivo']->idusuario;
				$sede = $_SESSION['userActivo']->idsede;
				$fictxtaccion = "ELIMINAR";
                
                $contenido = $_SESSION['userActivo']->usuario." - ".$_SESSION['userActivo']->paterno." ".$_SESSION['userActivo']->materno." ".$_SESSION['userActivo']->nombres.", está eliminando un cliente en la tabla TB_PAGANTE COD.".$txtcodigo;
				
                $rpta = $this->mpagante->m_eliminacliente(array($txtcodigo));
                if ($rpta == 1) {
                	$auditoria = $this->mauditoria->insert_datos_auditoria(array($usuario, $fictxtaccion, $contenido, $sede));
                    $dataex['status'] = true;
                    $dataex['msg']    = 'Cliente eliminado correctamente';
