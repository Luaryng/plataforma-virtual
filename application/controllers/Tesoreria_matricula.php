...
class Tesoreria_matricula extends CI_Controller {
  ...
  //√
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if (getPermitido("127")=='SI'){
                ...
                if ($this->form_validation->run() == false) {
                    ...
                    $periodo          = base64url_decode($this->input->post('periodo'));
                    $alumno          = $this->input->post('student');
                    ...
                    if ($rpta->salida == "1") {
                        ...
                        $txtestado = ($estado == "SI") ? "Bloqueando" : "Activando";
                        $fictxtaccion = "EDITAR";
                        $contenido = $_SESSION['userActivo']->usuario." - ".$_SESSION['userActivo']->paterno." ".$_SESSION['userActivo']->materno." ".$_SESSION['userActivo']->nombres.", está $txtestado la visualización de notas para $alumno en el periodo $periodo / tabla TB_MATRICULA COD.".$rpta->newcod;
    ...
    public function fn_activado_bloquea_acceso()
    {
        ...
        if (getPermitido("37")=='SI'){
            ...
            if ($this->form_validation->run() == FALSE){
                ...
                $this->musuario->m_activado(array($activo,$iduser));
                $resultado['status']=true;

                $usuario = $_SESSION['userActivo']->idusuario;
                $sede = $_SESSION['userActivo']->idsede;
                $fictxtaccion = 'EDITAR';
                $estado = ($activo == "SI") ? "Activando" : "Bloqueando";
                $contenido = $_SESSION['userActivo']->usuario." - ".$_SESSION['userActivo']->paterno." ".$_SESSION['userActivo']->materno." ".$_SESSION['userActivo']->nombres.", está ".$estado." a el usuario $alumno en la tabla TB_USUARIO COD.".$iduser;
                $auditoria = $this->mauditoria->insert_datos_auditoria(array($usuario, $fictxtaccion, $contenido, $sede));
                $resultado['accion'] = $estado;
            }
        }
        header('Content-Type: application/x-json; charset=utf-8');
