...
class Transparencia extends Error_views{
    ...
    public function vw_virtual_archivos($area,$id)
    {
        ...
        $fila=$this->mtransparencia->m_get_item(array($id,$area));
    ...
    public function vw_principal()
	{
		if (getPermitido("62")=='SI'){
			...
			$ahead= array('page_title' =>'Transparencia | ERP'  );
			$asidebar= array('menu_padre' =>'transparencia');
	...
	public function fn_search_data()
	{
		...
		if($this->input->is_ajax_request()){
			...
			if (count($rstdata) > 0) {
                ...
                $datos = $this->load->view('transparencia/result_data', $rsdata, true);
            } else {
	...
	public function vw_agregar()
	{
		if (getPermitido("64")=='SI'){
			$ahead= array('page_title' =>'Transparencia - Agregar | ERP'  );
			$asidebar= array('menu_padre' =>'transparencia');
			$this->load->view('head',$ahead);
			$this->load->view('nav');
			$this->load->view('sidebar_portal',$asidebar);
			$arraydts['categorias'] =$this->mcategoria_transparencia->m_filtrar_categorias_activasxtipo(array('TP'));
			$this->load->view('transparencia/vw_agregar', $arraydts);
			$this->load->view('footer');
		}
		else{
			 $this->vwh_nopermitido("NO AUTORIZADO - ERP");
	...
	public function vw_editar($area,$id)
    {
		...
		if (getPermitido("63")=='SI'){
			$area=base64url_decode($area);
        	$id=base64url_decode($id);
			$ahead= array('page_title' =>'Transparencia - Editar | ERP'  );
			$asidebar= array('menu_padre' =>'transparencia');
			$this->load->view('head',$ahead);
			$this->load->view('nav');
			$this->load->view('sidebar_portal',$asidebar);
			$fila['mat']=$this->mtransparencia->m_get_item(array($id,$area));
			$fila['categorias'] =$this->mcategoria_transparencia->m_filtrar_categorias_activasxtipo(array('TP'));
			//var_dump($fila['mat']);
			$this->load->view('transparencia/vw_agregar', $fila);
			$this->load->view('footer');
		}
		else{
			 $this->vwh_nopermitido("NO AUTORIZADO - ERP");
			...
			if (getPermitido("56")=='SI'){
				...
				else
				{
	        		...
	        		if ($rpta->salida==1){
						...
						$dataex['redirect'] =base_url()."portal-web/transparencia?tp=$categoria";
	        		}
	...
	public function fn_delete_file(){
        ...
        if ($this->input->is_ajax_request()) {
            if ($_SESSION['userActivo']->tipo != 'AL'){
                ...
                $this->mtransparencia->m_update_delfile(array($codigo));
                $dataex['status'] =true;
        ...
        if ($this->input->is_ajax_request()) {
        	...
        	if (getPermitido("65")=='SI'){
	            if ($_SESSION['userActivo']->tipo != 'AL'){
	                ...
	                $fila=$this->mtransparencia->m_get_item(array($codigo,$area));
	                if (isset($fila->codigo)){
	                	$this->mtransparencia->m_delete(array($codigo));
	                	$dataex['status'] =true;
		                if ("" !== $fila->ruta){
		                    $link    = $fila->ruta;
		                    $pathtodir =  getcwd() ; 
