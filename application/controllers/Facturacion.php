...
class Facturacion extends Error_views{
	...
	public function fn_datos_doc_cliente()
	{
		$this->form_validation->set_message('min_length', '* {field} debe tener al menos {param} caracteres o digite %%%%%%%%.');
		$this->form_validation->set_message('max_length', '* {field} debe tener al menos {param} caracteres.');
		$dataex['status'] =FALSE;
		...
		if ($this->input->is_ajax_request())
		{
			...
			$codigo = base64url_decode($this->input->post('vw_fcb_codigo'));
			$rstipos=array();
			$rpta = $this->mfacturacion->get_docpago_x_codigo(array($codigo));
			...
			$rpta->codigo64 = base64url_encode($rpta->codigo);
	...
	public function fn_update_datos_pagante()
	{
		$this->form_validation->set_message('required', '%s Requerido');
        $dataex['status'] =FALSE;
        $dataex['msg']    = '¿Que Intentas?.';
        if ($this->input->is_ajax_request())
        {
            ...
            $this->form_validation->set_rules('vw_dp_txt_codocument','Codigo','trim|required');
            $this->form_validation->set_rules('vw_dp_txt_dnipagante','Nro documento','trim|required');
            $this->form_validation->set_rules('vw_dp_txt_dpagante','Pagante','trim|required');
            $this->form_validation->set_rules('vw_dp_txt_direccion_pag','Dirección','trim|required');
            $this->form_validation->set_rules('vw_dp_txt_codigpagante','codigo Pagante','trim|required');
            ...
            else
            {
                $dataex['msg'] ="No se eliminó ";
                $dataex['status'] =FALSE;
                $codigo = base64url_decode($this->input->post('vw_dp_txt_codocument'));
                $docpagante = $this->input->post('vw_dp_txt_dnipagante');
                $pagante = $this->input->post('vw_dp_txt_dpagante');
                $direcpagante = $this->input->post('vw_dp_txt_direccion_pag');
                $codpagante = $this->input->post('vw_dp_txt_codigpagante');
                $tipodocum = $this->input->post('vw_dp_txt_tipdocpagante');
                                
                $rpfila=$this->mfacturacion->fn_update_datospagante(array($codigo,$codpagante,$pagante,$tipodocum,$docpagante,$direcpagante));
                if ($rpfila->salida == 1) {
