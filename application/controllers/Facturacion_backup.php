...
class Facturacion extends Error_views{
	...
	public function vw_documentos_crear()
	{
		if ($_SESSION['userActivo']->codnivel!="3"){
			...
			if (isset($serie_sede->sede)){
				...
				$rstdata['estados']=$this->mfacturacion->m_get_estado_mat();
	...
	public function fn_filtrar_xsede()
	{
		...
		if($this->input->is_ajax_request()){
			...
			$rstdata = $this->mfacturacion->m_get_emitidos_limit_xsede($inicio,$limite,$_SESSION['userActivo']->idsede,$busqueda,$databuscar);
				...
				else
				{
					...
					if ($n_items_recibidos>0){
						...
						if ($tipodoc_cod=="BL") {
							...
							$dinscript = $this->mfacturacion->m_get_inscripcion_por_carne(array($pagante_cod));
							$dmatricula = $this->malumno->m_matriculas_x_inscripcion(array($dinscript->idinscripcion));
						}
							...
							if ($rpta->salida == '1') {
								...
								foreach ($items as $key => $item) {
									...
									if ($tipodoc_cod=="BL") {
										...
										if (($gestion_cod == "01.01") || ($gestion_cod == "01.02")) {
		                		...
		                		$dataex['matestado'] = $statusmat;
		                		$dataex['codmat'] = base64url_encode($codmat);
							}
							else{
								$dataex['status'] =FALSE;
	...
	public function fn_get_datos_matricula()
	{
		$this->form_validation->set_message('required', '%s Requerido');
		$this->form_validation->set_message('min_length', '* {field} debe tener al menos {param} caracteres.');
		$this->form_validation->set_message('max_length', '* {field} debe tener al menos {param} caracteres.');
		...
		{
			$dataex['msg'] ='Intente nuevamente o comuniquese con un administrador.';
			$this->form_validation->set_rules('fgi-txtmat','codigo','trim');
			if ($this->form_validation->run() == FALSE)
			{
				$dataex['msg']="Existen errores en los campos";
				$errors = array();
			...
			{	
				...
				$busqueda = base64url_decode($this->input->post('fgi-txtmat'));
				$rsfila=$this->mfacturacion->m_get_matriculas_xcodigo(array($busqueda));
				$dataex['status'] =TRUE;
				if (!is_null($rsfila)){
					$rsfila->idinsc=base64url_encode($rsfila->idinsc);
					$datereg =  new DateTime($rsfila->fecha);
					$rsfila->fecha = $datereg->format('Y-m-d');
					$fila=$rsfila;
	...
	public function fn_insert_fac_matricula()
    {
        $this->form_validation->set_message('required', '%s Requerido');
        $this->form_validation->set_message('min_length', '* {field} debe tener al menos {param} caracteres.');
        $this->form_validation->set_message('is_unique', '* {field} ya se encuentra registrado.');
        ...
        if ($this->input->is_ajax_request())
        {
            ...
            else
            {
                ...
                $dataex['status'] =FALSE;
                $fmcodmat64=$this->input->post('fm-txtidmatricula');
                $fmcodmat = base64url_decode($fmcodmat64);
                ...
                $docpago = $this->input->post('fm-txtcodpago');
                $estadom = $this->input->post('fm-cboestado');
                ...
                if ($fmcodmat64 == "0") {
                 	$rsrow=$this->mmatricula->m_insert(array($fmtxtid,$fmcbtipo,$fmcbbeneficio,$fmcbperiodo,$fmtxtcarrera,$fmcbciclo,$fmcbturno,$fmcbseccion,$fmtxtcuota,$estadom,$fmtxtfecmatricula,$fmtxtobservaciones,$fmcbplan,$fmtapepaterno,$fmtapematerno,$fmtnombres,$fmtsexo));
                 } else {
                 	$rsrow=$this->mmatricula->m_update_mat(array($fmcodmat,$fmtxtid,$fmcbtipo,$fmcbbeneficio,$fmcbperiodo,$fmtxtcarrera,$fmcbciclo,$fmcbturno,$fmcbseccion,$fmtxtcuota,$estadom,$fmtxtfecmatricula,$fmtxtobservaciones,$fmcbplan,$fmtapepaterno,$fmtapematerno,$fmtnombres,$fmtsexo));
                 }
                ...
                elseif ($rs==-1) {
                    ...
                    if ($fmcodmat64 == "0") {
                    	$rptupd = $this->mfacturacion->fn_update_doc_codmatricula(array($newcod,base64url_decode($docpago)));
                    }
                    $dataex['vdata'] =$cursos;
                }
            }
