...
class Mfacturacion extends CI_Model {
  ...
  public function m_get_tiposdoc()
  {
    ...
    //janior todo en 1
    public function m_get_emitidos_limit_xsede($inicio, $limite,$sede,$tipo,$pagante='',$fechas=array())
    {
        if ($inicio == 0) {
        ...
        $qry = $this->db->query("SELECT 
          ...
          WHERE tb_docpago.sede_id=? AND tb_docpago.tipodoc_cod like ? $sqltext 
          ORDER BY
            tb_docpago.dcp_id DESC $limitado", $data);
        ...
        $qryc = $this->db->query("SELECT 
          ...
          FROM
            tb_docpago
          WHERE tb_docpago.sede_id=? AND tb_docpago.tipodoc_cod like ?  $sqltext", $data);
        $rs['numitems']= $qryc->row()->conteo;
        return $rs;
    }


    /*public function m_get_emitidos_filtro_xsede_limit($inicio, $limite, $sede,$pagante,$fechas)
    {
      $data=array($sede);
      $sqltext="";
      if (count($fechas)>0){
      ...
      $qry = $this->db->query("SELECT 
          ...
          FROM
            tb_docpago_sunat
            INNER JOIN tb_docpago ON (tb_docpago_sunat.dcps_id = tb_docpago.dcp_id) 
            WHERE tb_docpago.sede_id=? $sqltext order by tb_docpago.dcp_id DESC
             $limitado", $data);
  ...
  public function get_docpagos_pendientes($data) {
    $qry = $this->db->query("SELECT 
      ...
      WHERE
        ...
        tb_docpago.dcp_fecha_creacion < CURDATE()
      ORDER BY
