...
class Mpagante extends CI_Model
{
	...
	public function m_get_pagantes($data){
        $resultdoce = $this->db->query("SELECT 
			...
			WHERE tb_pagante.pg_tipo_doc =? AND tb_pagante.pg_dni_ruc=? LIMIT 6", $data);
    ...
    public function m_get_pagante_search($data){
        ...
        if ($data[0] != "" AND $data[1] == "") {
            $items = "WHERE tb_pagante.pg_dni_ruc = $data[0]";
        } else if ($data[0] == "" AND $data[1] != "") {
            $items = "WHERE tb_pagante.pg_razon_social like '%$data[1]%'";
        } else if ($data[0] != "" AND $data[1] != "") {
            $items = "WHERE tb_pagante.pg_dni_ruc = $data[0] AND tb_pagante.pg_razon_social like '%$data[1]%'";
        }
        $result = $this->db->query("SELECT 
                  tb_pagante.pg_id as id,
                  tb_pagante.pg_codpagante as codpag,
                  ...
                  tb_pagante.pg_correo1 as email,
                  tb_pagante.pg_correo2 as email2,
                  tb_pagante.pg_correo_corp as ecorpo,
                  tb_pagante.pg_direccion as direccion,
                ...
                $items ORDER BY tb_pagante.pg_id DESC");

        return $result->result();
    }

    public function m_get_pagante_lista($data){
        $items = "";
        if ($data[0] != "" AND $data[1] != "" AND $data[2] == "") {
            $items = "WHERE tb_pagante.pg_tipo_doc = '$data[0]' AND tb_pagante.pg_dni_ruc = $data[1]";
        } else if ($data[0] != "" AND $data[1] == "" AND $data[2] != "") {
            $items = "WHERE tb_pagante.pg_razon_social like '%$data[2]%'";
        } else if ($data[0] != "" AND $data[1] != "" AND $data[2] != "") {
            $items = "WHERE tb_pagante.pg_tipo_doc = '$data[0]' AND tb_pagante.pg_dni_ruc = $data[1] AND tb_pagante.pg_razon_social like '%$data[2]%'";
        }
        $result = $this->db->query("SELECT 
                  tb_pagante.pg_id as id,
                  tb_pagante.pg_codpagante as codpag,
    ...
    public function m_filtrar_pagante($data){
      ...
      FROM
        ...
        INNER JOIN tb_provincia ON (tb_distrito.cod_provincia = tb_provincia.prv_codigo)
        INNER JOIN tb_departamento ON (tb_departamento.dep_codigo = tb_provincia.cod_departamento)
      WHERE CONCAT(tb_pagante.pg_dni_ruc,tb_pagante.pg_razon_social) like ? LIMIT 6",$data);
