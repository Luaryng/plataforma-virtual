...
if(!function_exists('getNotasAlumnos')){
	function getNotasAlumnos($miembro,$indicadores,$evaluaciones,$alumno) {
	            ...
	            //************************* FIN CANCHAQUE
	                ...
	                foreach ($evaluaciones as $evaluacion) {
	                    if ($evaluacion->tipo=="C"){
	                    //$nindicador=str_pad($evaluacion->indicador, 2, "0", STR_PAD_LEFT);
	                    $pc=$alumno[$miembro->idmiembro]['eval'][$evaluacion->indicador]['CC']['nota'];
	                    if (!is_numeric($pc)) $pc=0;
	                    $ta=$alumno[$miembro->idmiembro]['eval'][$evaluacion->indicador]['PC']['nota'];
	                    if (!is_numeric($ta)) $ta=0;
	                    $ei=$alumno[$miembro->idmiembro]['eval'][$evaluacion->indicador]['AP']['nota'];
	                    if (!is_numeric($ei)) $ei=0;
	                    

	                    $alumno[$miembro->idmiembro]['eval'][$evaluacion->indicador]['PI']['tipo'] = $evaluacion->tipo; 
	                    $alumno[$miembro->idmiembro]['eval'][$evaluacion->indicador]['PI']['nota']= round(($pc * 0.3) + ($ta * 0.4) + ($ei * 0.3),0); 
	                    }
	                }
	                //PROMEDIO FINAL
	                $nco=0;
	            ...
	            //************************* HUARMACA
	            //EVALAUCIONES CALCULADAS POR ALUMNO 
	            else if (getDominio()=="iestphuarmaca.edu.pe"){
	                $nco=0;
	                //PROMEDIO FINAL
	                $pis=0;
	                ...
	                foreach ($indicadores as $indicador) {
	                    ...
	                    $pisr=(($ep * 3) + ($ta * 4) + ($ef * 3))/10; 
	                    $pir2=round($pisr,2);
	                    $pir1=round($pir2,1);
	                    $pi=round($pir1,0);
	                    $rci=floatval($alumno[$miembro->idmiembro]['eval'][$indicador->codigo]['RC']['nota']);
	                    $pi=($rci>$pi)?$rci:$pi;
	            ...
	            //************************* CHARLES
	            //EVALAUCIONES CALCULADAS POR ALUMNO 
	            else if (getDominio()=="charlesashbee.edu.pe"){
	                $nco=0;
	                $eis=0;
	                $tais=0;
	                foreach ($indicadores as $indicador) {
